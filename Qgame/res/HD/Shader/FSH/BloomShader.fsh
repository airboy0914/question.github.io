...
void main() {
    vec4 originalTextureColor = texture2D(CC_Texture0, v_texCoord);
    vec3 resultColor = vec3(0.0);

    // 計算3*3的像素質，需要考慮環境光的影響
    for (int i = 0;i<9;++i){
        vec3 textureColor = texture2D(CC_Texture0, v_texCoord + vec2(textureSize.x * (float(i/4)-1.0), textureSize.y * (float(i - (4 * (i/4))) -1.0))).rgb;
